...
# 读取数据
data=pd.read_csv("dataset/wine_data.csv")
# 分析数据相关性
...
class net(torch.nn.Module):
    def __init__(self):
        ...
        self.linear_0 = torch.nn.Linear(13, 16)
        self.linear_1 = torch.nn.Linear(16, 8)
        self.linear_2 = torch.nn.Linear(8, 4)
        self.linear_3 = torch.nn.Linear(4, 3)
    def forward(self,x):
...
for epoch in range(epoch_num):
    ...
    for x,y in tqdm.tqdm(iter(train_loader)):
        ...
        running_loss=running_loss+loss/32
        opt.zero_grad()
        loss.backward()
        opt.step()
    ...
    print("val {} epoch ,total acc is {}%,time use {}s".format(epoch + 1, acc*100, end - start))
